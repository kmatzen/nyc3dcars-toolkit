...
def compute_car_pose(photo, x1, y1, x2, y2, angle, types):
    ...
    newDir = numpy.array([[(det_middle - det_width/2) / det_focal],
    ...
    middle_ray = numpy.array([[(middle_x*photo.width - det_width/2)/det_focal],
                     [(det_height/2 - middle_y*photo.height)/det_focal],
                     [-1]])

    middle_ray_enu = car_enu.T.dot(R.T.dot(middle_ray))

    top_ray = numpy.array([[(middle_x*photo.width - det_width/2)/det_focal],
    ...
    for t in types:
        ...
        b = numpy.array([
            [-left_rot[0][0]*left_ray_enu[1][0] + left_rot[1][0]*left_ray_enu[0][0]],
            [-right_rot[0][0]*right_ray_enu[1][0] + right_rot[1][0]*right_ray_enu[0][0]],
        ])
        ...
        top_point = distance*top_ray_enu/numpy.linalg.norm(top_ray_enu)#math.sqrt(top_ray_enu[0][0]**2 + top_ray_enu[1][0]**2)
        bottom_point = distance*bottom_ray_enu/numpy.linalg.norm(bottom_ray_enu)#math.sqrt(bottom_ray_enu[0][0]**2 + bottom_ray_enu[1][0]**2)

        #bottom_point = (bottom_point + top_point - numpy.array([[0],[0],[t.tight_height]]))/2
        ...
        #bottom_point = (x[1][0]*bottom_ray_enu + x[1][0]*top_ray_enu - numpy.array([[0],[0],[t.tight_height]]))/2

        #bottom_point = left_right_position
        bottom_point = (bottom_point+left_right_position)/2
        #bottom_point = left_right_position
        

#        bottom = top_point[2] - 0.3048*t.model.height
        bottom = bottom_point[2]
        ...
        pglla1 = func.ST_SetSRID(func.ST_MakePoint(lla1[1], lla1[0], lla1[2]), 4326)
        pglla2 = func.ST_SetSRID(func.ST_MakePoint(lla2[1], lla2[0], lla2[2]), 4326)
        pglla3 = func.ST_SetSRID(func.ST_MakePoint(lla3[1], lla3[0], lla3[2]), 4326)
        pglla4 = func.ST_SetSRID(func.ST_MakePoint(lla4[1], lla4[0], lla4[2]), 4326)
        ...
        pglla = func.ST_SetSRID(func.ST_MakePoint(lla[1], lla[0], lla[2]), 4326)
...
def detect(pid, model_filename):
    ...
    try:
        nDets, = session.query(func.count(nyc3dcars.Detection.id)) \
        ...
        image = scipy.misc.imread(os.path.join(nyc3dcars.photo_dir, photo.filename))
        pyramid = pydro.features.BuildPyramid(image, pydro_model.features.sbin, pydro_model.interval, pydro_model.features.extra_octave, pydro_model.maxsize[1]+1, pydro_model.maxsize[0]+1)
        ...
        for i, tree in enumerate(parse_trees):
            ...
            for lla, geom, type, world_angle in compute_car_pose(photo, x1, y1, x2, y2, angle, types):
                det = nyc3dcars.Detection(
                    ...
                    y2=float(y2),
                    score=float(score),
                    prob=float(1.0/(1.0 + math.exp(model.a*score + model.b))),
                    model=model,
                    angle=angle,
                    lla=lla,
                    geom=geom,
